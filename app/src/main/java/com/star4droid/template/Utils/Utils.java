...
public class Utils {
	...
	public static BitmapFont generateFontFrom(FileHandle jsonFileHandle){
		boolean available = jsonFileHandle.exists() && !jsonFileHandle.isDirectory();
		return generateFontFrom(available ? jsonFileHandle.parent() : null,available ? jsonFileHandle.readString() : "");
	}
			...
			if(propertySet.containsKey(field.getName()))
				try {
					...
					if(type.contains("float")){
						field.set(parameter,propertySet.getFloat(field.getName()));
					} else if(type.equals("int") || type.contains("integer")){
						field.set(parameter,propertySet.getInt(field.getName()));
					} else if(field.get(parameter).getClass().isEnum()){
						Field enm = field.get(parameter).getClass().getField(propertySet.getString(field.getName()));
						field.set(parameter,enm);
					} else if(type.contains("color")){
						field.set(parameter,propertySet.getColor(field.getName()));
